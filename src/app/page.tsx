...
export default function Home() {
  ...
  async function launch() {
    ...
    setLoading(true); setError(""); setResult(null); setEventId(null); setClaim(null); setView("observe");
    try {
  ...
  return <main className="shell">
    <header className="topbar">
      ...
      <div className="top-actions"><button className={judge ? "selected" : ""} onClick={() => setJudge(!judge)}><Gauge size={14} />Judge mode</button><a href="https://github.com/vatshalpandey-commits/proofpilot" target="_blank" rel="noreferrer">Source <ArrowUpRight size={13} /></a></div>
    </header>

    <section className="mission-bar"><label><Search size={17} /><input aria-label="Research mission" value={question} onChange={e => setQuestion(e.target.value)} onKeyDown={e => { if ((e.ctrlKey || e.metaKey) && e.key === "Enter") launch(); }} /></label><button className={`chaos ${chaos ? "on" : ""}`} onClick={() => setChaos(!chaos)}><FlaskConical size={14} />Chaos <span>{chaos ? "ON" : "OFF"}</span></button><button className="launch" onClick={launch} disabled={loading}>{loading ? <LoaderCircle className="spin" size={15} /> : <Zap size={15} />}{loading ? "Investigating" : "Launch run"}</button></section>
    ...
    <nav className="mobile-tabs">{(["observe", "trace", "report", "compare"] as View[]).map(x => <button className={view === x ? "selected" : ""} onClick={() => setView(x)} key={x}>{x}</button>)}</nav>
...
function Constellation({ result, loading, claim, activeEvent, selectEvent }: { result:Result|null; loading:boolean; claim:number|null; activeEvent:TraceEvent|null; selectEvent:(id:string)=>void }) {
  const obs=result?.state.observations??[]; const failed=obs.filter(x=>!x.result.ok); const state=loading?"PLANNING":activeEvent?.type==="recovery"?"RECOVERING":result?"COMPLETE":"READY";
  return <div className={`constellation ${claim!==null?"tracing":""}`}>
    ...
    <svg viewBox="0 0 900 620" preserveAspectRatio="none" aria-hidden="true"><path className="route violet" d="M430 310C340 250 270 180 175 145"/><path className="route blue" d="M470 295C575 230 635 150 742 142"/><path className={`route mint ${result?"lit":""}`} d="M485 335C610 355 657 424 742 463"/><path className={`route mint evidence ${result?"lit":""}`} d="M430 350C340 405 280 456 170 476"/>{failed.length>0&&<><path className="route failed" d="M475 315C590 290 650 268 748 250"/><path className="route recover" d="M748 250C650 300 610 350 690 408"/><g className="interrupt"><circle cx="625" cy="280" r="12"/><path d="M620 275l10 10M630 275l-10 10"/></g></>}{(loading||result)&&<circle className="packet" r="4"><animateMotion dur="1.6s" repeatCount={loading?"indefinite":"1"} path="M470 295C575 230 635 150 742 142"/></circle>}</svg>
    <div className={`core ${state.toLowerCase()}`}><i/><i/><div><BrainCircuit size={24}/><span>DECISION CORE</span><b>{state}</b><small>{loading?"Choosing next action":result?`${result.state.step} decisions recorded`:"Awaiting a mission"}</small></div></div>
    ...
    {failed.length>0&&<button className="failure" onClick={()=>{const e=result?.state.trace.find(x=>x.type==="recovery");if(e)selectEvent(e.id)}}><AlertTriangle size={13}/><span>FAILED ROUTE</span><b>Recovery selected</b></button>}
    <div className="caption"><b>{claim!==null?`Tracing claim ${claim+1}`:"Live execution topology"}</b><span>{claim!==null?"Related provenance illuminated":"Select an event to inspect it"}</span></div>
  </div>;
}
